...
def get_XGeS_from_mulskips(rundirname, XGeS_tmp, tstep, 
    cell_map, rank_map, wall_map, V, dofmap, comm,
    alat, xBox0, yBox0, zBox0, lenx, leny, lenz,
    coofilename=None, driver=None):
    ...
    # Note: dofs can belong to multiple cells.
    t0 = time.time()
    # Initialize new fem vector (should not be zeros! But a previously defined Function, to keep possible XGeS user-defined gradients extending beyond KMC box size!
    #XGeS_tmp = Function(V)
    ...
    tmp_array_Ge = np.zeros(np.shape(XGeS_tmp_array))
    tmp_array_Si = tmp_array_Ge.copy()
    for izj in KMCnotwall:
        if MPI.rank(comm) == rank_map_flat[izj]:
