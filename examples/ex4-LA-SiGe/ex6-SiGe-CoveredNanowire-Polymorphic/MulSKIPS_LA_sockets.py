...
# For the oxide below no liquid is considered because we never melt it. Below we update only the FEM arrays related to solid 
epsOxR_c = "2.245"
epsOxI_c = "0"
rh0Ox_c = "2203"
kOx_c = "( ( ( 1.005E-2 + 1.298E-5 * T ) * (0.5*(1.0+tanh((1170-T)/slp))) + 2.512E-2 * (0.5*(1.0+tanh((T-1170)/slp))) ) * 1E2 )"
cpOx_c = "(( 0.7074 + 2.989E-4 * T ) * 1000) * (0.5*(1.0+tanh((2000-T)/slp))) + 1305.2 * (0.5*(1.0+tanh((T-2000)/slp)))"
#######################################################################
#######################################################################
...
epsR_c.append(epsSiR_c)
epsR_c.append(epsSiR_c)
epsR_c.append(epsOxR_c)

epsI_c=[]
epsI_c.append(epsSiI_c)
epsI_c.append(epsSiI_c)
epsI_c.append(epsSiI_c)
epsI_c.append(epsOxI_c)
...
mpclass = pmul.process.PhaseChange('SiGe_X', calibration_params=calibration_params) # if not provided as argument, the calibration is internally fixed
...
if initialState == 'tripleSF':
    ...
    Seed_box = [lenz-LenVac_KMC, 240, 240, 240, 120] # Better if all of these are divisible by 12 ! lunghezza SF 144 
...
#### NW.geo
regions=[20,10,1,10] # it is regions=[0,10,1,10] in geo 
...
pmul.setuprun.setup_mulskips_src(execpath, lenx, leny, lenz, socketport=port) 
...
cell_map, rank_map, wall_map = pmul.io.send_geom_to_mulskips(regions, mesh, V, dofmap, mv, comm,
...
while t <= t_stop:
    ...
    if KMCstartflag and KMCregrowth_incomplete:  
        ...
        if rank==0: 
            pmul.setuprun.run_mulskips(setup_only=True, driver=driver, \
                execpath=execpath, runpath='.', \
                Simulation='LA', mp=mpclass, \
                PtransZig=0.5, RunType='R', IDUM=9117116, \
        ...
        t0 = time.time()
        pmul.io.send_T_for_mulskips(t, T_curr, \
        ...
        Ph0 = Ph_curr
        Ph_curr, Nsolid_KMC = pmul.io.get_phases_from_mulskips('.', \
        ...
        XGe_curr = XGeS
        XGeS = pmul.io.get_XGeS_from_mulskips('.', XGe_curr, \
            t, cell_map, rank_map, wall_map, V, dofmap, comm, \
            alat, xBox0+xOff, yBox0+yOff, zBox0+zOff, lenx, leny, lenz, \
            coofilename=None, driver=driver) 
        MPI.barrier(comm)
        ...
        try:
            os.system(f'mv I*[0-9].xyz I*[0-9]_d.xyz start.dat InterpXGeS_* InterpPhase_* {rundirname}/')
        ...
        try:
            os.system('rm I*[0-9]_v.xyz I*[0-9].src emfield_* Temperature_* ') 
