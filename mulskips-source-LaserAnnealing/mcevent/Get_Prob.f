...
C               write(*,*)'Absorbing',IndexEvent_Abs
     ...
     >                    Index_Event,Prob_Event) ! Occ=1/0 NSiC 1=01 Si, 2=10 C
      ...
      INTEGER i,j,Index_Event,SiOrC,NSiC,N_C,N_Si,N_wall,CovIndN,k         ! WALL added CovInd
      INTEGER CovInd
...
!            write(*,*)Occ,NSiC,CovInd,Coor,Site,NextN
     ...
     >        PosT,LenT) ! K
          ...
          DO k=1,NCrystal
            ! Melting
            PE(k,:,:,:,:,:,:) = P0(k) 
     >            * EXP(-PtransE(k,:,:,:,:,:,:)/(kb*temperature)) 
     >            * merge(1.d0,0.d0,PtransE(k,:,:,:,:,:,:).NE.0.d0)        ! Set PE to zero for all array elements not specified in start.dat (e.g. (1,0,0) or (1,3,1))
            ! Solidification
            PD(k,:) = 0.5d0 * P0(k) * FLOAT(CountLiquid(k))/SUM(CountLiquid) 
     >            * EXP(-PtransD(k,:)/(kb*Tm(k))) 
     >            * (1.0d0 + ERF((temperature - Tflex(k)) / sigma(k))) 
     >            * merge(1.d0,0.d0,PtransD(k,:).NE.0.d0)        ! Set PD to zero for all array elements not specified in start.dat (so this determines whether 2nd or third crystal species will be deposited)   
          END DO
        ELSE
          PE = PtransE
          PD = PtransD
