...
!  Filled wall Sites at z=0 (helps with migrating vacancies issue)
      DO z=0,5
       ...
       END DO
      END DO      
!  Filled Bulk Sites in 100 SiGex lattice
!      write(*,*)'15 9 45',LattCoo(15,9,45)
      DO z=6,Len-4
        ...
        DO x=0,LenX-1
         IF(MOD(x,3).EQ.0.AND.MOD(y,3).EQ.0.AND.MOD(z,3).EQ.0)THEN
          ...
          IF((MOD(x1,2).EQ.MOD(y1,2)).AND.(MOD(x1,2).EQ.MOD(z1,2)))THEN
           IF(MOD(x1+y1+z1,4).EQ.0)THEN ! random Si / Ge
            rr = RAND()
           ...
           ELSE IF(MOD(x1+y1+z1,4).EQ.3)THEN ! random Si / Ge
            rr = RAND()
            IF(rr.GE.alloyfraction)THEN
                LattCoo(x,y,z)=IBSET(LattCoo(x,y,z),PosSi)
                CountCrystal(1) = CountCrystal(1) + 1
       ...
       END DO
      END DO
!  Filled  Sites in 100 SiGex lattice MC particles..NumAdatoms
      DO z=Len-3,Len-1
       DO y=0,LenY-1
        DO x=0,LenX-1
         IF(MOD(x,3).EQ.0.AND.MOD(y,3).EQ.0.AND.MOD(z,3).EQ.0)THEN
...
!  Empty  Sites in 100 SiGex lattice (to be also consider MC particles in NumAdatoms)
      DO z=Len,Len
       DO y=0,LenY-1
        DO x=0,LenX-1
         IF(MOD(x,3).EQ.0.AND.MOD(y,3).EQ.0.AND.MOD(z,3).EQ.0)THEN
