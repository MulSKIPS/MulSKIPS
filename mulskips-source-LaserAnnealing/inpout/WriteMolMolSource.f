...
***********************************************************************
      ...
      ! Write out crystal species counts 
      IF(NCrystal.GT.1)THEN
        write(*,*)'SUM(CountCrystal)', SUM(CountCrystal)
      END IF
      write(*,*)'CountCrystal', CountCrystal
      write(*,*)'xCrystal', FLOAT(CountCrystal)/SUM(CountCrystal)
      write(*,*)'CountLiquid', CountLiquid
      write(*,*)'xLiquid', FLOAT(CountLiquid)/SUM(CountLiquid)
      
      IF(Iter.GT.1)THEN
        !write(*,*)'CountCrystalOld', CountCrystalOld
        diff=FLOAT(CountCrystal-CountCrystalOld)
        write(*,*)'diffCrystal', diff
        write(*,*)'Atom fraction from last check:', diff/
     >                     (SUM(CountCrystal)-SUM(CountCrystalOld))
      ...
      IF(NCov.NE.0)THEN
        IF(NCov.GT.1)THEN
          write(*,*)'SUM(CountCov)', SUM(CountCov)
        END IF
